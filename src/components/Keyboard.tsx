...
import pressedNotesStore from "../stores/pressedNotes"
...
export default class Keyboard extends Component<KeyboardProps> {
	...
	getKeyboardStyle({ dragging, offset }): React.CSSProperties {
		...
		return {
			...
			transform: `translateX(${offset.x -
				rootNoteIndex * SizeStore.keyboardButtonSize.get()}px)`,
	...
	getPlayableNotes() {
		return this.props.scaleStore.notes.get().reduce((acc, on, note) => {
			if (on) {
				acc.push(note)
			}
			return acc
	...
	viewButtons({ dragging }) {
		...
		return repeat(playableNotes, 8).map((note, index) => {
			const isRoot =
				...
				modPos(semitoneOffset, semitonesPerOctave)
